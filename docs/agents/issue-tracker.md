...
- **Blocking**: GitHub's native issue dependencies are canonical. Add an edge with `gh api --method POST repos/<owner>/<repo>/issues/<child>/dependencies/blocked_by -F issue_id=<blocker-db-id>`, using the blocker's numeric database id, not its issue number or node id. Where dependencies are unavailable, use `Blocked by: #<n>, #<n>` at the top of the child body.
- **Frontier query**: list open map children, remove those with open blockers or assignees, and select the first remaining child in map order.
